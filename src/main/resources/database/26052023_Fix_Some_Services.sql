...
create table `user`(
                       ...
                       `phone` varchar(255),
                       `avatar` varchar(255),
                       `is_status` bit default 1 check(`is_status` = 0 or `is_status` = 1),
...
values
    ...
    ('Phong','kakashi7','$2a$12$3StsBnHAgc9gnLhm1nIpUeQzdtf0SpdDiFTEsF9M2YQr0TAKoKmSq','g@codegym.com',1,'https://res.cloudinary.com/dr3rwgzpl/image/upload/v1684729641/Product/cute-panda-with-coffee-cartoon-illustration-vector_dog4rb.jpg'),
...
alter table `package_details`
    add constraint `FK_package_details_centers` foreign key (center_id) references centers (`id`);

alter table `package_details`
    add constraint `FK_package_details_packages` foreign key (package_id) references packages (`id`);

alter table `services`
    add constraint `FK_service_package_detail` foreign key (package_detail_id) references package_details (`id`);

alter table `centers`
    add constraint `FK_center_user` foreign key (user_id) references `user` (`id`);
...
alter table `package_reviews`
    add constraint `FK_package_review_package` foreign key (package_detail_id) references package_details (`id`);
alter table `package_reviews`
